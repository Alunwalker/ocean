...
{
	return texture(noiseSampler3D, vec3(uv , time * wave_speed)).x * matu;
}
void vertex()
{
	...
	float time = TIME;
	float h = texture(noiseSampler3D, vec3(uv1 , time * wave_speed)).x * matu;
	
	NORMAL = normalize(vec3(h - height(uv1 + vec2(0.01, 0.0), time), 0.1, h - height(uv1 + vec2(0.0, 0.01), time)));
	VERTEX.y += h * matu;
...
//	ALBEDO = texture(testTex,uv1).xyz;
